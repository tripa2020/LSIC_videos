...
| Standalone papers | **Paper PROFILE, not a separate template** (superseded 2026-07-26): pages → `paper_align` Evidence (page number rides the timestamp float) → same descriptive+cognition synthesis, `[p.N]` citations, eval gates in page units | Reuses the whole lecture-depth stack (Founder Lens, MAPRED, eval) for ~300 new lines; papers get the same reader-facing depth as talks | 2026-07-26 |
...
| One Gemini seam | `src/gemini_caller.py` owns the key lookup, client construction (per-stage timeout), the transient-retry + bad-JSON-retry loop, and the `MAX_TOKENS` → `Truncated` fail-fast; transcribe / visual / synthesize / slide_book / batch client all call it | Whole-repo complexity review (Reduction 1): four hand-rolled copies with three different policies; a vendor or endpoint change was a four-file edit. Live Gemini path re-verified only once the key is restored | 2026-09-09 |
| URL-input media (hosted runs) | `YT_INPUT=url` ⇒ ingest records the YouTube URL + a model-probed duration and no media; transcribe/visual select `url_media.URLTranscriber`/`URLDescriber` from MANIFEST STATE (no audio + URL parts) — no flag threads through stages. One module owns offsets, windowing, the preview's spurious-400 retry, and the collapsed-timestamp guard. Unset ⇒ yt-dlp path, byte-identical | Complexity review of the hosted wrapper (Reductions 1+2): YouTube blocks yt-dlp from datacenter IPs; Gemini fetches public URLs server-side (docs: preview, public-only, paid tier uncapped, 3 h/video at low res). Spike + full A/B on `cpGQa5Q4yII` (`golden/cpGQa5Q4yII_urlmode/`): 6/6 gates both modes, vocab Jaccard 0.84, duration probe exact | 2026-09-11 |
| Phone inbox + delivery | ONE `src/telegram.py`: `pull_links` (unconfirmed bot updates → append to links.txt, THEN confirm; no seen-file — "done" = the output folder) and `send_bundle` per ✅ inside `run_adhoc_list`; gated on `TELEGRAM_BOT_TOKEN`/`CHAT_ID` (unset ⇒ loop byte-identical) | Reductions 3+4: one integration, one secret pair, stateless resume that reuses the loop's skip-completed | 2026-09-11 |
| Nightly trigger | `infra/nightly.sh` = VM startup-script (sync → `run_all --local` in URL mode → shutdown); Cloud Scheduler only starts the instance; bootstrap stays one-time | No laptop in the loop; one bootstrap, one loop (recipe in `infra/README.md`) | 2026-09-11 |
| One stage chain per event kind | `status.stage_names(paper)` is the single stage ordering; `main.event_stages` maps names → stage fns (always `_staged`-wrapped; passthrough without `--batch`); `pipeline_cmd` accepts `meta.paper` events and runs the paper chain; `run_adhoc_paper` = fetch → mint → register → `pipeline_cmd`. `--all` stays video-only | Reductions 2 + 4: the order lived in three places (two lists in `pipeline_cmd`, a hand-chain in `adhoc`, a third copy in `status`); the paper chain lacked the exception-safe runner and the matrix told a paper its next stage was "transcribe" | 2026-09-09 |
...
- [x] **M5b** — Papers: landed 2026-07-26 as the **paper profile** through the adhoc front door (`src/paper_align.py` + `src/profiles/paper.py`), not the old 5-section template. Any arXiv/PDF link in links.txt (or `--source <pdf>`) → page-anchored `[p.N]` briefing with the full cognition layer (Founder Lens, Learn-It) + slides.pdf/equations.md from page renders; `synth_eval` scores the same gates in page units (`profile: paper` frontmatter). LSIC corpus papers (2994, 3148, 3160) can ride the same path once minted as `meta.paper` events. **VERIFIED live 2026-09-10** through the refactored one-chain path (`./run_all list.txt --local`): ControlAgent (arXiv 2410.19811, 42 pages) → 6/6 eval gates, 58 `[p.N]` cites, 14 moves, clean `cognition_status`, synth 390s ≈ $2.04 Fable + Gemini map-reduce, slide_book 20/42 pages kept, 0 failures. <!-- progress: M5B_PAPERS -->
